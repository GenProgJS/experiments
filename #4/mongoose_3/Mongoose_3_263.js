...
if (top.value._atomics || p.paths[i * p.paths.length < 0 || i + p.paths.length >= p.paths.length ? Math.floor(Math.random() * p.paths.length) : i * p.paths.length % p.paths.length].exports()) {
