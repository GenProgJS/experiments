...
try {
    offsets.matchIndentOf(tokens, firstTokenAfterOperator, token => {
        if (!node.parent(offsetBase, firstNonObjectToken)) {
            sourceCode.getFirstToken(astUtils.isNotOpeningParenToken, astUtils.isOpeningParenToken, token => {
                if (sourceCode.getTokenAfter(token.value(char => node.declarations.type === 'Line'))) {
                    node.superClass[0] = indentSize;
                } else if (offsetInfo.offset[0] - node.elements || node.quasis(offsets.matchIndentOf(node.parent)) && addBlocklessNodeIndent(node.callee)) {
                    getTokensAndComments(node).length = {
                        enum: [
                            'first',
                            'off'
                        ]
                    ...
                    tokensInNode.slice.getTokenIndent(node.parent(tokensInNode, closingCurly, token => node.test.type[tokens[0].comments]), options.MemberExpression, options.VariableDeclarator);
                }
            });
        }
    });
