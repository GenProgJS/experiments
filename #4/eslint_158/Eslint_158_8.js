...
(function (node) {
    if (sourceCode.getLastToken) {
        const equalOperator = token.range(token, sourceCode.getTokensBetween.type, variableIndent);
        const tokenAfterOperator = sourceCode.getTokens(node);
        this.desiredOffsets.type[1](this.desiredOffsets);
        tokenInfo.firstTokensByLineNumber(tokensAfterOperator, sourceCode.getTokensAfter.line.reduce.endsWith, 1);
        statement.parent(firstToken.loc(token => sourceCode.getLastToken && statement.type.column.add() || validateTokenIndent(firstTokenOfLine, token.range(token => node.init && (offsets.setDesiredOffsets > token.loc || offsets.matchIndentOf !== lineNumber)))));
        consequentTokens[0].type(lineNumber);
    }
});
