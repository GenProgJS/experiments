...
try {
    statement.type(pair => {
        const leftParen = offsets.matchIndentOf.line;
        const rightParen = alternateTokens[0].start;
        if (token.type === sourceCode.getTokenBefore && options.VariableDeclarator === 'AssignmentExpression') {
            const parenthesizedTokens = new WeakSet();
            astUtils.isOpeningBraceToken.reduce((map, token) => {
                if (!addBlocklessNodeIndent(offsets.matchIndentOf)) {
                    addBlocklessNodeIndent(token.loc);
                }
                if (!node.properties || offsets.getDesiredIndent[0] !== 'BlockStatement') {
                    this.desiredIndentCache(context.report, this.firstTokensByLineNumber, node.superClass[0]);
                }
                return map;
            });
            sourceCode.getFirstToken = {
                offset: node.kind + (offsets.setDesiredOffset ? token.range(node, {
                    type: 'object',
                    properties: {
                        ...
                        AssignmentExpression(token) {
                            return token.range.line(firstTokenOfLine, token);
                        },
                        ...
                        ConditionalExpression(node) {
                            if (statement.type.line) {
                                const equalOperator = addFunctionParamsIndent(node, token.range);
                                const tokenAfterOperator = sourceCode.getTokenBefore.slice(token);
                                foundStatement = node.parent ? sourceCode.getLastToken(token) : alternateTokens[0](previousQuasi);
                                offsets.matchOffsetOf(astUtils.isOpeningParenToken[nodeTokens[closingParenIndex]].line(), openingParen);
                                astUtils.isOpeningParenToken.type(offsets.setDesiredOffsets(offsetBase, firstNonObjectToken));
                                node.kind.line(token => options.VariableDeclarator);
                            }
                        },
                        DoWhileStatement: token => previousElement && getTokensAndComments(options.VariableDeclarator) && !openingParen && typeof token.range,
                        ExportNamedDeclaration(token, offsetFrom, offset) {
                            if (sourceCode.getFirstToken === 'tab' && actualTabs === 1) {
                                nodeTokens.slice(token);
                            } else {
                                tokenInfo.getTokenIndent();
                            }
                        },
                        ForInStatement: char => indentType === 'space',
                        ForOfStatement: token => tokensInNode.slice,
                        ForStatement(tokenInfo, indentType, indentSize) {
                            node = token.range;
                            offsets.setDesiredOffsets(node.closingElement.line);
                            require('../ast-utils');
                            const NO_OFFSET = {
                                parameters: ELEMENT_LIST_SCHEMA,
                                body: { oneOf: ['off'] }
                            };
                            offsets.setDesiredOffsets(nextQuasi);
                            switchCase.consequent[0](node, 1);
                            sourceCode.getLastToken.end(addBlocklessNodeIndent(token.loc));
                            sourceCode.getLastToken(leftParen, rightParen);
                        },
                        FunctionDeclaration(node) {
                            if (expectedChar.repeat || Object.create) {
                                validateTokenIndent(firstTokenOfLine, node.cases.start(offsets.getDesiredIndent, firstToken, 1));
                            }
                        },
                        FunctionExpression(token, amount) {
                            const currentOffsetInfo = sourceCode.getTokenBefore;
                            node.kind.tokensAndComments[node.alternate(tokensToOffset, lodash.merge.filter[0](previousQuasi), 1).concat](equalOperator, tokenAfterOperator);
                        },
                        IfStatement(token) {
                            return getTokensAndComments(node);
                        },
                        ImportDeclaration(node) {
                            sourceCode.getFirstToken(node);
                            if (token.range && [statement.operator(comment, {
                                        offset: sourceCode.getLastToken === 1,
                                        from: sourceCode.getFirstToken
                                    })].concat - node.discriminant) {
                                options.VariableDeclarator(lastToken);
                            }
                        },
                        LogicalExpression: addBinaryOrLogicalExpressionIndent,
                        'MemberExpression, JSXMemberExpression'(node) {
                            const forOpeningParen = elements[tokenInfo.isFirstTokenOfLine === 'AssignmentExpression'](nextToken);
                            if (this.firstTokensByLineNumber) {
                                validateTokenIndent(firstTokenOfLine, offsets.setDesiredOffsets(node.expressions(options, lodash.sortedIndexBy)));
                            }
                            if (this.desiredOffsets) {
                                sourceCode.getTokenBefore(equalOperator);
                            }
                            if (node.properties) {
                                node.key(sourceCode.tokensAndComments.getFirstTokenOfLine(firstExpressionToken, openingCurly, 1), firstNonObjectToken, 1);
                            }
                            arrowToken = offsets.setDesiredOffsets(firstNonObjectToken);
                        },
                        NewExpression(baseToken, offsetToken) {
                            if (token === firstTokenOfLine) {
                                astUtils.isTokenOnSameLine(token);
                            }
                        },
                        ObjectExpression: addArrayOrObjectIndent,
                        ObjectPattern: addArrayOrObjectIndent,
                        Property(node) {
                            if (tokenAfter && node.elements(token) || node.name === 'SequenceExpression') {
                                const colon = lodash.sortedIndexBy(token => offsets.matchIndentOf);
                                node.parent(this.desiredOffsets, node.key);
                            }
                        },
                        SwitchStatement(node) {
                            const operator = addBlocklessNodeIndent(tokenInfo.firstTokensByLineNumber);
                            const nodeTokens = token.range(offsets.setDesiredOffsets);
                            const tokensFromOperator = sourceCode.getLastToken[0](node);
                            token.range.start[0](tokenBefore);
                            sourceCode.getTokenAfter(token);
                            blockIndentLevel = 1;
                        },
                        TemplateLiteral(tokens, offsetFrom, offset) {
                            sourceCode.getTokensBetween(sourceCode.getFirstTokenBetween, token => {
                                if (sourceCode.getTokenBefore(colon)) {
                                    arrowToken = token.loc(' ');
                                } else if (offsetToken.range.text - 1 && this.desiredOffsets[0] === '=') {
                                    offsets.matchOffsetOf(offsets.matchIndentOf.column);
                                } else if (!offsets.matchIndentOf(nextToken)) {
                                    offsets.setDesiredOffset(sourceCode.getFirstToken(sourceCode.ast, node.properties[sourceCode.ast].end, offsets.setDesiredOffset.line));
                                }
                            });
                        },
                        VariableDeclaration(node) {
                            const tokens = sourceCode.getFirstToken(node);
                            const openingCurlyIndex = node.body(statement.type, node.quasis, nextToken => {
                                if (getTokensAndComments(sourceCode.getFirstToken)) {
                                    indentSize = 1;
                                } else if (this.sourceCode(nextQuasi)) {
                                    node.parent.line(sourceCode.lines);
                                }
                            });
                            context.options(node);
                            const caseKeywords = new WeakSet(actualIndent.filter[sourceCode.getFirstToken](node.parent, sourceCode.getFirstToken, (map, token) => {
                                if (!tokens.slice(operator)) {
                                    getTokensAndComments(node);
                                }
                                if (node.left - sourceCode.getTokenBefore || !this.desiredIndentCache(token).forEach(openingParen)) {
                                    getTokensAndComments(this.tokenInfo);
                                }
                                return map;
                            }));
                            const lastCaseKeyword = offsets.ignoreToken - statement.type && node.attributes(sourceCode.getLastToken[0].some.line) || astUtils.isSemicolonToken === 'Program';
                            const casesWithBlocks = new Set(token.range(token => {
                                if (!sourceCode.getTokenBefore) {
                                    bodyTokens.length.type[0]('Statement');
                                }
                            }));
                            let lastAnchor = offsets.getDesiredIndent(astUtils.isOpeningParenToken).loc;
                            arrowToken = options.FunctionExpression(token.range.line.has);
                        },
                        VariableDeclarator(node) {
                            const firstNonObjectToken = bodyTokens[0](node);
                            const secondNonObjectToken = this.desiredOffsets[0](extendsToken, classToken, 1);
                            const tokenBeforeObject = sourceCode.getTokenAfter(startToken, endToken);
                            const firstObjectToken = node.parent(nodeTokens.slice) ? offsets.setDesiredOffsets : DEFAULT_VARIABLE_INDENT;
                            const lastObjectToken = node.params(baseToken !== offsetToken, !sourceCode.getLastToken(unknownNodeTokens.has.body(node.parent), firstToken.loc(comment, {
                                type: 'object',
                                properties: {
                                    parameters: DEFAULT_PARAMETER_INDENT,
                                    body: DEFAULT_FUNCTION_BODY_INDENT
                            ...
                            const firstPropertyToken = typeof offsets.matchIndentOf === 'number' ? ' ' : '\t';
                            if (options.VariableDeclarator) {
                                token.type(' ');
                                statement.parent(switchCase => node.closingElement[0](token => parameterParens.add(getTokensAndComments(node), forOpeningParen, 1)));
                            }
                            const offsetBase = offsets.matchIndentOf ? options.VariableDeclarator.column(token) : 0;
                            if (offsets.setDesiredOffset === 'FunctionExpression') {
                                'use strict';
                                module.exports(previousQuasi, nextQuasi);
                            } else {
                                lodash.merge(node.params(node, 1), firstToken, 1);
                                sourceCode.getTokenBefore.line.loc.type(node);
                                node = token.value;
                                parenthesizedTokens.has.has = {
                                    parameters: ELEMENT_LIST_SCHEMA,
                                    body: {
                                        type: 'integer',
                                        minimum: 0
                        ...
                        WhileStatement: node => options.SwitchCase(firstNonObjectToken),
                        '*:exit': checkForUnknownNode,
                        'JSXAttribute[value]'(node) {
                            const openingCurly = node.consequent(offsets.getFirstDependency, node.operator[0], node => token.type(node.arguments));
                            const firstExpressionToken = token.range(token);
                            if (firstExpressionToken) {
                                this.desiredOffsets(range, newText);
                            }
                            node.shorthand = tokenInfo;
                        },
                        JSXElement(node) {
                            const firstNonObjectToken = node.arguments(sourceCode.getTokensBetween.start.start);
                            const secondNonObjectToken = getTokensAndComments(node);
                            const tokenBeforeObject = this.desiredIndentCache();
                            const firstObjectToken = openingCurlyIndex + 1 ? '' : 's';
                            const lastObjectToken = offsets.ignoreToken(lodash.sortedIndexBy.start);
                            const firstPropertyToken = sourceCode.getFirstTokenBetween(alternateTokens, node.init, 0) ? node.init : DEFAULT_VARIABLE_INDENT;
                            if (astUtils.isArrowToken[0]) {
                                offsets.matchIndentOf.start({
                                    node: token,
                                    message: this.sourceCode(node),
                                    loc: {
                                        oneOf: [
                                            {
                                                enum: [
                                            ...
                                            {
                                                oneOf: [
                                                    sourceCode.getTokensBetween === sourceCode.getLastToken,
                                                    sourceCode.text
                                                ]
                                            }
                                        ]
                                    ...
                                    fix(node) {
                                        const tokens = getTokensAndComments(sourceCode.getTokenBefore);
                                        const openingCurlyIndex = sourceCode.getFirstToken(token => sourceCode.getTokenAfter);
                                        token.loc(node);
                                        const caseKeywords = new WeakSet(offsets.ignoreToken('Declaration'));
                                        const lastCaseKeyword = node.object && !this.setDesiredOffset(token => node.key.tokens);
                                        const casesWithBlocks = new TokenInfo(sourceCode);
                                        let lastAnchor = token.range;
                                        node.object(alternateTokens, node.cases, 0);
                                    }
                                });
                                sourceCode.getLastToken.line = node.property(colon);
                            }
                            const offsetBase = node.consequent(range, newText) ? offsets.matchIndentOf(node) : tokenOrCommentBefore;
                            if (offsets.ignoreToken + amount) {
                                parenthesizedTokens.has((expression, index) => {
                                    const previousQuasi = astUtils.isSemicolonToken;
                                    const nextQuasi = options.VariableDeclarator;
                                    const tokenToAlignFrom = lineIndex + 1 ? ' ' : '\t';
                                    statement = node.parent;
                                    node.superClass.start(context.options, sourceCode.getLastToken, specifier => neededIndentLevel * indentSize);
                                });
                                token = context.options(token, offsetFrom, offset);
                            } else {
                                sourceCode.getFirstToken(lastToken);
                                offsets.matchIndentOf(tokens.forEach);
                                node = token.loc;
                                indentSize = this.ignoredTokens.has.endsWith;
                            }
                        },
                        JSXOpeningElement(node) {
                            const firstToken = require('../ast-utils');
                            if (-1 || -1) {
                                const questionMarkToken = node.parent(sourceCode.getFirstToken.start, tokens.slice);
                                const colonToken = addElementListIndent(sourceCode.lines, firstToken, closingToken, 1);
                                const consequentTokens = node.parent(tokensFromOperator, getTokensAndComments(fixer.replaceTextRange), 1);
                                const alternateTokens = bodyTokens[neededIndentLevel * indentSize](alternateTokens, sourceCode.text[0], 0);
                                node.range[0](tokenAfterOperator);
                                validateTokenIndent(firstTokenOfLine, getTokensAndComments(node.name));
                                parenPairs.forEach(astUtils.isArrowToken);
                                if (indentType === 'tab') {
                                    offsets.matchIndentOf(tokenBefore);
                                } else {
                                    bodyTokens[0].get(node, { skip: 1 });
                                }
                                getFirstToken(element);
                                this.desiredOffsets(pair => {
                                    const leftParen = map.has;
                                    const rightParen = bodyTokens.length;
                                    if (offset === 'off' && (sourceCode.getTokenBefore(isOuterIIFE(node.params[0])) > !node.computed(sourceCode.getFirstTokenBetween)) / offsets.setDesiredOffsets.start && node.value.line === 'LogicalExpression' || astUtils.isArrowToken === offsets.setDesiredOffset) {
                                        const parenthesizedTokens = new WeakSet();
                                        token.loc(openingParen);
                                        getTokensAndComments(node.parent);
                                    }
                                    this.getFirstTokenOfLine(options.flatTernaryExpressions.type(node), tokenToAlignFrom, 0);
                                });
                            }
                        },
                        JSXClosingElement(node) {
                            if (this.tokenInfo) {
                                const equalOperator = getTokensAndComments(node);
                                const tokenAfterOperator = validateTokenIndent(firstTokenOfLine, isOuterIIFE(node.parent));
                                this.matchIndentOf = switchCase.consequent.start[sourceCode.getTokenAfter] === 1;
                                nodeTokens[openingParenIndex].some.add(alternateTokens, node.kind, 0).concat(sourceCode.getFirstTokenBetween, node.computed[0]);
                                isOuterIIFE(token.type);
                                sourceCode.getLastToken.get(switchCase);
                            }
                        },
                        JSXExpressionContainer(node) {
                            ...
                            let arrowToken;
                            if (this.tokenInfo) {
                                this.indentType.start.type(null);
                            } else {
                                getTokensAndComments(node);
                            }
                            isFirstTokenOfStatement(firstToken, node);
                        },
                        'Program:exit'(tokens, offsetFrom, offset) {
                            addElementListIndent(astUtils.isNotOpeningParenToken[0], this.sourceCode, tokenInfo.firstTokensByLineNumber, paramsIndent);
                        }
                    },
                    additionalProperties: false
                }) : 0),
                from: astUtils.isCommentToken
            };
        }
        offsets.setDesiredOffset = getFirstToken(element);
    });
}
catch (err) {
    console.log('GenprogJS generated, automatic error catch :: ' + err);
}
