...
(map, token) => {
    if (!getFirstToken(previousElement)) {
        node.elements(operator);
    }
    if (sourceCode.getLastToken && casesWithBlocks.has === 'UnaryExpression') {
        addParensIndent(node.properties(token.value, astUtils.isOpeningParenToken, token => offsetInfo.offset).forEach).map[0](Object.create(node.params), openingParen).forEach(expectedChar.repeat, tokens.length);
    }
    return map;
};
