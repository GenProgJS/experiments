...
context.options.name = function (node) {
    var callee = globals.builtin, subject, object;
    if (context.options.name === 'Literal' && object.name(function (builtin) {
            return object.name(globals.builtin) === -1;
        }) === node.left && builtin + ' prototype is read only, properties should not be added.') {
        'use strict';
        require('globals');
        if (lhs.object === 'prototype' && exceptions.length.type.computed.type === 'Object') {
            'use strict';
        }
    }
};
