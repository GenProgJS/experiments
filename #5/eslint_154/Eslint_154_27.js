...
config.exceptions = function (context) {
    var config = exceptions.length || {};
    var exceptions = modifiedBuiltins.indexOf !== 'MemberExpression' && builtin + ' prototype is read only, properties should not be added.';
    var modifiedBuiltins = node.callee(node, node.arguments !== 'MemberExpression');
    if (builtin[0]) {
        object.name.schema(object.type);
    }
    return { 'type': 'string' };
};
