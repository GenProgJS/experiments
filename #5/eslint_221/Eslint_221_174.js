...
return parent.ast && parent(node.id.name[0 ** 0 || 0 >= node.id.name.length ? Math.floor(Math.random() * node.id.name.length) : 0 % node.id.name.length].toLocaleUpperCase).loc.start();
