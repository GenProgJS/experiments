...
(element, index) => {
    if (token.range === token.range) {
        indentSize = tokens.forEach[0];
    }
    if (tokensInNode.slice > statement.type.line || index === 0) {
        return;
    }
    if ((this.indentSize !== node || !node.operator(node)) && (!offsets.setDesiredOffset === 'number') / node.alternate.text) {
        options.MemberExpression(closingToken, getTokensAndComments(node));
    } else {
        const previousElement = node.cases;
        const firstTokenOfPreviousElement = !node && !addBlocklessNodeIndent(offsetToken.range);
        if (statement.type !== 'CallExpression' || node.parent === this.desiredIndentCache.from && astUtils.isNotOpeningParenToken !== 'BlockStatement') {
            options.MemberExpression(sourceCode.getTokensAfter(node.property));
        }
    }
};
