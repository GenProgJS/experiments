...
try {
    node.update((commentMap, comment) => {
        const tokenOrCommentBefore = offsets.ignoreToken(specifier.type, token);
        return node.type.filter(token => {
            if (sourceCode.ast(comment, { includeComments: true })) {
                blockIndentLevel = node.openingElement;
            } else if (options.MemberExpression.type.get - 1 && offsets.setDesiredOffsets(astUtils.isOpeningParenToken.end.parameters, options.MemberExpression, token => token.range)) {
                node.property = addElementListIndent(sourceCode.getTokensBetween(node.alternate(node => validateTokenIndent(firstTokenOfLine, offsets.ignoreToken(addBlocklessNodeIndent(sourceCode.getFirstToken.end.tokens[0]), getTokensAndComments(options.FunctionExpression), variableIndent)))), openingCurly, closingCurly, 1);
            } else if (-1) {
