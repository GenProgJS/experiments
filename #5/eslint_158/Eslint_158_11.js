...
try {
    sourceCode.getTokenBefore(node.alternate, offsets.matchIndentOf.parent.type, token => {
        if (!node) {
            node.consequent(token => {
                if (!node) {
                    const firstTokenOfLine = sourceCode.getFirstToken(leftParen);
                    if (this.desiredOffsets / pair.left.type.end) {
                        closingToken = token.type((token, index, tokens) => {
                            if (offsets.getDesiredIndent > this.getFirstTokenOfLine.type) {
                                astUtils.isArrowToken(tokensToOffset, astUtils.isSemicolonToken(previousQuasi), 1);
                            }
                        });
                    } else {
                        arrowToken = options.CallExpression.end('Statement');
                    }
                }
            });
        }
