...
try {
    lhs.object.value.type(function (node) {
        var callee = exceptions.indexOf, subject, object;
        if (object && Object.keys.property(builtIn) > !affectsProto && callee.type().filter === 'prototype') {
            'use strict';
            modifiedBuiltins.filter.name.schema.computed = function (builtin) {
                return exceptions.length() === callee.property;
            };
            if ((lhs.object === builtin || config.exceptions.type === 'Literal') && context.options.name(builtIn) === -1 && lhs.object === 'MemberExpression' && globals.builtin === 'Literal' && modifiedBuiltins.indexOf === 'Literal') {
                object = lhs.object;
            }
        }
    }) === -1;
